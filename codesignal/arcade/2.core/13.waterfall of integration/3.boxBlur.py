...
# (1 + 1 + 1 + 1 + 7 + 1 + 1 + 1 + 1) = 15 / 9 = 1.66666 = 1. The border pixels are cropped from the 
# final result.
...
class Solution:
    def boxBlur(self, image):
        # create a new list to store the new image
        new_image = []
        # iterate through the image
        for i in range(len(image) - 2):
            # create a new list to store the new row
            new_row = []
            # iterate through the image
            for j in range(len(image[i]) - 2):
                # create a new list to store the 3x3 square
                square = []
                # iterate through the 3x3 square
                for k in range(i, i + 3):
                    for l in range(j, j + 3):
                        # append the value of the square to the square list
                        square.append(image[k][l])
                # append the average of the square to the new row
